...
parser.add_argument("-c", "--cores", type=int, default=1, help="number of cores available for delphes, i.e. how many output files there should be.")
...
inputfile="%s/%s/%s/%s" % (args.base, args.tag, madgraphpath, orighepmc)

# figure out how many events in the input .hepmc file.
proc=subprocess.Popen(["grep \"E \" %s | wc" % inputfile],
                      shell=True,
...
with hep.open(inputfile) as f:
    ...
    while not f.failed():
        if evtcount%neventsperfile == 0:
            filecounter+=1
            outputfile="%s/%s/%s/%s" % (args.base, args.tag, madgraphpath, newhepmc % filecounter)
